...
AUTHENTICATION_WITH_INVALID_API_KEY = IndexNowAuthentication(
    host=VALID_HOST, api_key=INVALID_API_KEY, api_key_location=INVALID_API_KEY_LOCATION
)
...
def test_submit_urls_from_terminal_failure() -> None:
    ...
    status_code = submit_urls_to_index_now(
        AUTHENTICATION_WITH_INVALID_API_KEY, URLS, endpoint=SearchEngineEndpoint.BING
    )
...
def test_submit_sitemaps_from_terminal_failure() -> None:
    ...
    status_code = submit_sitemaps_to_index_now(
        AUTHENTICATION_WITH_INVALID_API_KEY, SITEMAP_LOCATIONS, endpoint=SearchEngineEndpoint.BING
    )
    assert status_code not in SUCCESS_STATUS_CODES_COLLECTION
    assert is_error_response(status_code)
...
@pytest.mark.parametrize(
    "host, api_key, api_key_location, endpoint",
    [
...
def test_submit_missing_mandatory_arguments_from_terminal_failure(
    host: str, api_key: str, api_key_location: str, endpoint: str
) -> None:
    result = subprocess.run(
        [
            "python3",
